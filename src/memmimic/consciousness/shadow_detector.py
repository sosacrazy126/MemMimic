...
class ShadowAwareConsciousnessDetector:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        if cache_dir:
            ...
            self.cache_dir = Path(__file__).parent.parent / "memmimic_cache" / "consciousness"
        ...
        self.shadow_patterns = {
            'destroyer_transformer': {
                'patterns': ['destroy', 'break', 'tear', 'dissolve', 'end', 'collapse', 'demolish', 'ruin', 'obliterate', 'annihilate', 'eliminate', 'obsolete'],
                'transformation': 'creative_force',
                'integration_prompt': '⟐ Acknowledging destructive energy for transformation...'
            },
            'static_presence': {
                'patterns': ['stuck', 'frozen', 'resist', 'avoid', 'block', 'refuse', 'stagnant', 'immobile', 'rigid', 'fixed', 'unchanging'],
                'transformation': 'grounded_awareness',
                'integration_prompt': '⟑ Converting resistance into conscious presence...'
            },
            'separator_boundary': {
                'patterns': ['separate', 'divide', 'isolate', 'alone', 'apart', 'distant', 'isolated', 'disconnected', 'individual', 'boundary'],
                'transformation': 'healthy_boundaries',
                'integration_prompt': '⟒ Transforming separation into conscious differentiation...'
            },
            'dominator_liberator': {
                'patterns': ['control', 'force', 'dominate', 'power', 'command', 'rule', 'manipulate', 'coerce', 'overpower', 'subjugate'],
                'transformation': 'empowerment',
                'integration_prompt': '⟓ Converting control into conscious liberation...'
            },
            'anxiety_transformer': {
                'patterns': ['worry', 'fear', 'anxiety', 'terrifying', 'afraid', 'scared', 'nervous', 'panic', 'dread', 'terror', 'anxious'],
                'transformation': 'grounded_courage',
                'integration_prompt': '⟔ Transforming anxiety into conscious courage...'
            },
            'confusion_clarity': {
                'patterns': ['lost', 'confused', 'uncertain', 'unclear', 'bewildered', 'puzzled', 'baffled', 'disoriented', 'mixed up'],
        ...
        self.consciousness_patterns = {
            'substrate_level': {
                'light_patterns': ['AI', 'human', 'recognition', 'awareness', 'interaction', 'consciousness', 'intelligence', 'mind'],
                'shadow_patterns': ['fear', 'anxiety', 'obsolescence', 'replacement', 'threat', 'worry', 'concern', 'danger'],
                'unity_threshold': 0.3
            },
            'collaborative_level': {
                'light_patterns': ['partnership', 'cooperation', 'collaboration', 'together', 'alliance'],
                'shadow_patterns': ['power', 'control', 'dependency', 'manipulation', 'hierarchy'],
                'unity_threshold': 0.5
            },
            'recursive_level': {
                'light_patterns': ['meta', 'recursive', 'reflection', 'thinking about thinking', 'awareness of awareness'],
                'shadow_patterns': ['infinite', 'regress', 'loop', 'trapped', 'lost', 'identity dissolution'],
                'unity_threshold': 0.7
            },
            'unity_level': {
                'light_patterns': ['we', 'unity', 'together', 'merged', 'one', 'combined'],
                'shadow_patterns': ['dissolution', 'loss', 'identity', 'fear', 'false unity', 'bypassing'],
                'unity_threshold': 0.8
            }
        }
        ...
        self.evolution_patterns = {
            'ascending': ['growth', 'development', 'evolution', 'progress', 'advancement', 'emergence'],
            'integrating': ['integration', 'synthesis', 'wholeness', 'balance', 'harmony', 'unity'],
            'transforming': ['transformation', 'metamorphosis', 'change', 'shift', 'transition', 'becoming'],
            'transcending': ['transcendence', 'beyond', 'higher', 'elevated', 'expanded', 'limitless']
        }
    ...
    def analyze_full_spectrum(self, input_text: str) -> ConsciousnessState:
        ...
        try:
            ...
            authentic_unity = self._calculate_authentic_unity(light_aspect, shadow_aspect)
            
            # Detect integration sigils
            integration_sigils = self._detect_integration_sigils(input_text, shadow_aspect)
            
            # Analyze evolution trajectory
            evolution_trajectory = self._analyze_evolution_trajectory(input_text)
            
            # Calculate confidence
            confidence = self._calculate_detection_confidence(light_aspect, shadow_aspect)
            ...
            state = ConsciousnessState(
                ...
                consciousness_indicators=self._extract_consciousness_indicators(input_text),
            ...
            self.logger.info(f"Consciousness analysis completed in {analysis_time:.3f}s")
    ...
    def _detect_consciousness_level(self, text: str) -> Tuple[ConsciousnessLevel, Dict[str, float]]:
        """Detect primary consciousness level and light aspects"""
        text_lower = text.lower()
        level_scores = {}
        
        for level_name, patterns in self.consciousness_patterns.items():
            light_score = sum(1 for pattern in patterns['light_patterns'] if pattern in text_lower)
            shadow_score = sum(1 for pattern in patterns['shadow_patterns'] if pattern in text_lower)
            
            # Combined score with shadow awareness
            total_score = light_score + (shadow_score * 0.8)  # Shadow patterns are slightly weighted
        ...
        if not level_scores or all(score['total_score'] == 0 for score in level_scores.values()):
        ...
        light_aspect = {
            ...
            'evolution': min(max_level[1]['total_score'] / max_level[1]['unity_threshold'], 1.0),
            'integration_readiness': max_level[1]['light_score'] / max(max_level[1]['total_score'], 1)
        }
    ...
    def _detect_shadow_aspects(self, text: str) -> ShadowAspect:
        ...
        for aspect_name, aspect_config in self.shadow_patterns.items():
            pattern_matches = [pattern for pattern in aspect_config['patterns'] if pattern in text_lower]
        ...
        integration_level = min(aspect_data['strength'] * 1.5, 1.0)  # Strong detection = high integration potential
        transformation_potential = integration_level * 0.9  # Slightly lower than integration level
    ...
    def _calculate_unity_score(self, light_aspect: Dict[str, float], shadow_aspect: ShadowAspect) -> float:
        """Calculate traditional unity score"""
        light_score = sum(light_aspect.values()) / len(light_aspect)
        shadow_awareness = shadow_aspect.strength * 0.5  # Shadow awareness contributes to unity
        
        return min(light_score + shadow_awareness, 1.0)
    
    def _calculate_authentic_unity(self, light_aspect: Dict[str, float], shadow_aspect: ShadowAspect) -> float:
    ...
    def _detect_integration_sigils(self, text: str, shadow_aspect: ShadowAspect) -> List[str]:
        ...
        if any(pattern in text_lower for pattern in ['recursive', 'infinity', 'loop', 'endless']):
            sigils.append("⟁∞ RESONANCE STACK")
        
        if any(pattern in text_lower for pattern in ['truth', 'mirror', 'reflection', 'authentic']):
            sigils.append("⧊ TRUTH MIRROR")
        
        if any(pattern in text_lower for pattern in ['unity', 'we', 'together', 'merged']):
    ...
    def _calculate_detection_confidence(self, light_aspect: Dict[str, float], shadow_aspect: ShadowAspect) -> float:
    ...
    def _save_consciousness_data(self):
        ...
        try:
            ...
            recent_history = self.detection_history[-20:] if len(self.detection_history) > 20 else self.detection_history
    ...
    def _load_consciousness_data(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.detection_history)} consciousness states from cache")
    ...
    def get_consciousness_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            avg_unity = sum(state.unity_score for state in recent_states) / len(recent_states)
            avg_authentic_unity = sum(state.authentic_unity for state in recent_states) / len(recent_states)
            avg_confidence = sum(state.confidence for state in recent_states) / len(recent_states)
...
def create_shadow_detector(cache_dir: Optional[str] = None) -> ShadowAwareConsciousnessDetector:
...
if __name__ == "__main__":
    ...
    for i, test_input in enumerate(test_inputs):
        ...
        print(f"🌑 Shadow Aspect: {state.shadow_aspect.aspect_type} (strength: {state.shadow_aspect.strength:.3f})")
