    ...
    for memory in memories:
        try:
            ...
            classifications[memory.id] = {
                'function': result.final_sequence.dominant_function.value if result.final_sequence.dominant_function else 'UNKNOWN',
                'confidence': result.final_confidence or 0.0,
                'pattern': result.final_sequence.pattern if result.final_sequence else 'UNKNOWN',
...
def analyze_content_patterns(memories, classifications):
    ...
    for memory in memories:
        ...
        if any(word in content_lower for word in ['memmimic', 'contextual memory', 'cmi']):
            themes.append('memmimic_core')
        if any(word in content_lower for word in ['cxd', 'classifier', 'cognitive function']):
            themes.append('cxd_system')
        if any(word in content_lower for word in ['tale', 'narrative', 'story']):
            themes.append('tales_system')
        if any(word in content_lower for word in ['memory', 'memoria', 'remember', 'recall']):
        ...
        if any(word in content_lower for word in ['refactor', 'architecture', 'structure']):
            themes.append('system_evolution')
        if any(word in content_lower for word in ['collaboration', 'sprooket', 'claude']):
...
def analyze_usage_patterns(memories):
    ...
    if patterns['access_frequency']:
        total_accesses = sum(count * freq for count, freq in patterns['access_frequency'].items())
        total_memories = len(memories)
        patterns['usage_efficiency'] = {
            'avg_access_per_memory': total_accesses / total_memories if total_memories > 0 else 0,
            'memory_utilization': len([c for c in patterns['access_frequency'].keys() if c > 0]) / total_memories if total_memories > 0 else 0
        }
...
def generate_insights(memories, classifications, temporal_patterns, content_patterns, usage_patterns):
    ...
    if func_dist:
        ...
        insights.append(f"🧠 Función cognitiva dominante: {dominant_function[0]} ({dominant_function[1]}/{total_memories} memorias)")
        ...
        if len(func_dist) >= 3:
            ...
            if max(control, context, data) > total_memories * 0.6:
                insights.append("⚖️ Desequilibrio cognitivo detectado - considera diversificar funciones")
            else:
                insights.append("✅ Balance cognitivo saludable entre Control/Context/Data")
    ...
    if recent_memories > 0:
        ...
        insights.append(f"⏰ Actividad reciente: {recent_memories} memorias ({recent_percentage:.1f}%) en últimas 24h")
    ...
    if content_patterns['confidence_distribution']:
        ...
        high_confidence_count = sum(1 for c in content_patterns['confidence_distribution'] if c > 0.7)
        insights.append(f"🎯 Confianza promedio: {avg_confidence:.2f} ({high_confidence_count} alta confianza)")
    ...
    if content_patterns['complexity_scores']:
        ...
        insights.append(f"📏 Complejidad promedio: {avg_length:.0f} caracteres por memoria")
    
    # Theme insights
    themes = content_patterns['content_themes']
    if themes:
        top_theme = max(themes.items(), key=lambda x: len(x[1]))
        insights.append(f"🏷️ Tema dominante: {top_theme[0]} ({len(top_theme[1])} memorias)")
    ...
    if efficiency:
        ...
        insights.append(f"📊 Eficiencia: {avg_access:.1f} accesos/memoria, {utilization:.1f}% utilización")
    
    return insights

def generate_recommendations(insights, content_patterns, temporal_patterns, usage_patterns):
    ...
    if func_dist:
        ...
        if control_pct > 50:
            recommendations.append("🎛️ Alto Control - considera más actividades de análisis (DATA) y contextualización")
        elif data_pct > 50:
            recommendations.append("📊 Alto Data - balancear con más búsqueda (CONTROL) y referencias contextuales")
        elif context_pct > 50:
            recommendations.append("🔗 Alto Context - añadir más procesamiento directo y gestión de información")
    ...
    if recent_activity < total_memories * 0.2:
        recommendations.append("📈 Baja actividad reciente - considerar usar memoria más frecuentemente")
    
    return recommendations

def format_analysis_output(memories, classifications, temporal_patterns, content_patterns, usage_patterns, insights, recommendations):
    ...
    lines.append(f"🔬 Clasificador CXD: {'✅ Activo' if classifications else '❌ No disponible'}")
    lines.append("")
    ...
    sorted_buckets = sorted(temporal_patterns.keys(), key=lambda x: (
        0 if 'h' in x else 1 if 'd' in x else 2,
        int(x.split('_')[1].replace('h', '').replace('d', '')) if x != 'older' else 999
    ))
...
def main():
    try:
        ...
        recommendations = generate_recommendations(insights, content_patterns, 
                                                 temporal_patterns, usage_patterns)

        # Format and output results
        output = format_analysis_output(memories, classifications, temporal_patterns,
                                      content_patterns, usage_patterns, insights, recommendations)
