...
def list_tales(tale_manager, category=None, limit=10):
    ...
    try:
        tales = tale_manager.list_tales(
            category=category
        )
        
        if not tales:
            return ["📚 No tales found.", "", "💡 Create your first tale with save_tale()"]
        ...
        for i, tale in enumerate(tales, 1):
            ...
            category_emoji = "🧠" if category_display.startswith('claude') else "🔧" if category_display.startswith('projects') else "📦"
            
            result_parts.append(f"{i:2d}. {category_emoji} {name}")
            result_parts.append(f"    📂 {category_display} | 📏 {size_str} | 🕐 {updated}")
            if usage_count > 0:
    ...
    try:
        ...
        if category_stats:
            ...
            for category, info in sorted(category_stats.items()):
                ...
                category_emoji = "🧠" if category.startswith('claude') else "🔧" if category.startswith('projects') else "📦"
                result_parts.append(f"  {category_emoji} {category}: {count} ({percentage:.1f}%)")
            result_parts.append("")
        
        # System info
        result_parts.append("🔧 SYSTEM INFO:")
        result_parts.append(f"  📦 Cache size: {stats.get('cache_size', 0)}")
        result_parts.append(f"  🔄 Structure version: {stats.get('structure_version', 'unknown')}")
        result_parts.append("")
...
def main():
    ...
    try:
        ...
        if args.stats:
            ...
            result_parts = search_tales(tale_manager, args.query, args.category, args.limit)
