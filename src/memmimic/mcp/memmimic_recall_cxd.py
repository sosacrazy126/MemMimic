...
# -*- coding: utf-8 -*-
"""
🧠 MEMMIMIC HYBRID MEMORY SEARCH v3.0 🧠
...
Evolved from Clay-CXD collaboration
"""
...
def display_golden_briefing(memory_store):
    ...
    try:
        ...
        briefing_parts = []
        briefing_parts.append("🏆 === GOLDEN MEMORY ACQUIRED! ===")
        briefing_parts.append("💎 QUIÉN SOY (referencias empíricas):")
        briefing_parts.append("")

        for i, golden in enumerate(golden_memories, 1):
            content = golden["content"]
            memory_id = golden["memory_id"]
            briefing_parts.append(f"  {i}. {content} [Ref: #{memory_id}]")

        briefing_parts.append("")
        briefing_parts.append("✨ Estas son TUS reglas empíricamente validadas.")
        briefing_parts.append("🎯 Úsalas sin miedo - son para TI, no para complacer.")
        briefing_parts.append("")
...
def get_wordnet_synonyms(
    word: str, lang: str = "spa", max_synonyms: int = 5
) -> Set[str]:
    """
        ...
        for synset in synsets:
            ...
            for lemma in synset.lemmas(lang=lang):
                ...
                if (
                    synonym != word_lower
                    and len(synonym) > 1
                    and len(synonym) < 20  # Avoid very long compound words
                    and synonym.replace(" ", "").isalpha()
                ):  # Only alphabetic
...
class MemMimicSemanticSearchEngine:
    """
    Semantic search engine for MemMimic memories using CXD infrastructure.
    Provides vector similarity search with cognitive function filtering.
    """

    def __init__(self, cache_dir: Optional[str] = None):
        # Use MemMimic-relative cache directory
        if cache_dir:
    ...
    def _initialize_cxd(self) -> bool:
        """Initialize CXD components with error handling."""
        try:
            logger.debug("Initializing MemMimic CXD semantic engine...")
            ...
            from memmimic.cxd.providers.embedding_models import create_embedding_model
            from memmimic.cxd.providers.vector_store import create_vector_store
            ...
            self.cxd_classifier = create_optimized_classifier(config=config)
            logger.debug("OptimizedMetaCXDClassifier initialized")

            # Get embedding model from semantic classifier
            if hasattr(self.cxd_classifier, "semantic_classifier"):
                self.embedding_model = (
                    self.cxd_classifier.semantic_classifier.embedding_model
                )
            else:
                # Fallback to creating embedding model directly
                self.embedding_model = create_embedding_model()
            logger.debug(
                f"Embedding model: {getattr(self.embedding_model, 'model_name', 'unknown')}"
            )

            # Create dedicated vector store for MemMimic memories
            ...
            logger.debug(
                f"Memory vector store created: {type(self.memory_vector_store).__name__}"
            )

            # Try to load existing index
            self._load_memory_index()

            logger.info("🧠 MemMimic CXD semantic engine initialized successfully")
            return True

        except Exception as e:
            logger.error(f"Failed to initialize MemMimic CXD semantic engine: {e}")
            return False

    def _get_memory_cache_path(self) -> Path:
        """Get path for memory index cache."""
        return self.cache_dir / "memmimic_memory_index"
    ...
    def _load_memory_index(self) -> bool:
        ...
        try:
            ...
            if self.memory_vector_store.load(cache_path):
                # Load metadata
                metadata_path = cache_path / "memmimic_metadata.pkl"
                ...
                logger.info(
                    f"Loaded memory index with {self.memory_vector_store.size} vectors"
                )
                return True
    ...
    def index_memories(self, memory_store, force_rebuild: bool = False) -> bool:
        """
        Index all memories from MemMimic database as embeddings.

        Args:
            memory_store: MemMimic MemoryStore instance
        ...
        try:
            ...
            logger.info(
                f"✅ Indexed {len(new_memories)} memories in {self.stats['index_build_time']:.2f}s"
            )
            return True
    ...
    def search_semantic(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            similarities, indices = self.memory_vector_store.search(
            ...
            logger.debug(
                f"Semantic search: {len(results)} results in {search_time:.1f}ms"
            )
...
SPANISH_STOPWORDS = {
    ...
    "memmimic",
    "clay",
    "cxd",  # Added project-specific stopwords
}
...
def expand_query_with_wordnet(query: str, max_synonyms_per_word: int = 3) -> List[str]:
    ...
    for word in content_words:
        ...
        if wordnet_available:
            try:
                synonyms = get_multilingual_synonyms(
                ...
                if synonyms:
                    logger.debug(
                        f"WordNet expanded '{word}' → +{len(synonyms)} synonyms"
                    )
...
def wordnet_enhanced_search(
    memory_store, query: str, limit: int = 10
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for memory_id, match_data in term_matches.items():
            ...
            original_score = (
                min(original_count / len(original_terms), 1.0)
                if original_terms
                else 0.0
            )
            synonym_score = min(
                synonym_count / max(len(expanded_terms) - len(original_terms), 1), 1.0
            )
        ...
        results.sort(
            key=lambda r: (r["original_matches"], r["lexical_score"]), reverse=True
        )
...
# =============================================================================
# COMBINED SCORING ENGINE - TRUE HYBRID FUSION v3.0
# =============================================================================


def combine_and_score_v3(
    semantic_results: List[Dict[str, Any]], wordnet_results: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    Combine semantic and WordNet lexical results with intelligent scoring v3.0.

    Enhanced philosophy for MemMimic:
    ...
    for result in wordnet_results:
        try:
            ...
            if content_hash in all_memories:
                ...
                all_memories[content_hash]["lexical_score"] = result.get(
                    "lexical_score", 0.0
                )
                all_memories[content_hash]["original_matches"] = result.get(
                    "original_matches", 0
                )
                all_memories[content_hash]["synonym_matches"] = result.get(
                    "synonym_matches", 0
                )
                all_memories[content_hash]["matched_terms"] = result.get(
                    "matched_terms", []
                )
                all_memories[content_hash]["wordnet_metadata"] = result.get(
                    "metadata", {}
                )
            else:
    ...
    for content_hash, data in all_memories.items():
        try:
            ...
            # ENHANCED SCORING ALGORITHM v3.0:
            ...
            combined_score = min(
                base_score + convergence_bonus + original_term_bonus, 1.0
            )
    ...
    if combined_results:
        convergence_count = sum(
            1 for r in combined_results if "convergence" in r["search_method"]
        )
        original_count = sum(1 for r in combined_results if r["original_matches"] > 0)

        logger.debug(f"Combined scoring v3.0: {len(combined_results)} total results")
        logger.debug(
            f"Convergence results: {convergence_count}, Original term matches: {original_count}"
        )

    return combined_results


def apply_cxd_filter(
    combined_results: List[Dict[str, Any]], function_filter: str, cxd_classifier
) -> List[Dict[str, Any]]:
    """
    ...
    for result in combined_results:
        ...
        if cxd_classifier:
            try:
                ...
                if hasattr(cxd_classifier, "classify_detailed"):
                    cxd_result = cxd_classifier.classify_detailed(content)
                    if (
                        cxd_result.final_sequence
                        and cxd_result.final_sequence.dominant_function
                    ):
                        cxd_function = cxd_result.final_sequence.dominant_function.value
                        cxd_confidence = cxd_result.final_confidence or 0.0
                else:
                    # Fallback to simple classify
                    cxd_result = cxd_classifier.classify(content)
                    if (
                        hasattr(cxd_result, "dominant_function")
                        and cxd_result.dominant_function
                    ):
                        cxd_function = cxd_result.dominant_function.value
                        cxd_confidence = (
                            getattr(cxd_result.dominant_tag, "confidence", 0.0)
                            if hasattr(cxd_result, "dominant_tag")
                            else 0.0
                        )
            except Exception as e:
        ...
        if (
            function_filter == "ALL"
            or cxd_function == function_filter
            or cxd_function == "UNKNOWN"
        ):
...
# =============================================================================
# TRUE HYBRID SEARCH ENGINE v3.0 - SEMANTIC + WORDNET FOR MEMMIMIC
# =============================================================================
...
def search_memories_hybrid(
    ...
    """
    TRUE HYBRID SEARCH v3.0: Always semantic + Always WordNet + Combined scoring.

    REVOLUTIONARY search function with NLTK WordNet integration for MemMimic:
    ...
    try:
        ...
        if not semantic_engine.index_built:
            ...
            logger.info(
                f"🔄 Auto-refreshing index: {indexed_count} → {total_memories} memories"
            )
            semantic_engine.index_memories(
                memory_store, force_rebuild=False
            )  # Incremental
        else:
    ...
    logger.debug(
        f"🧠 MEMMIMIC HYBRID SEARCH v3.0 - Query: '{query}' | WordNet: {wordnet_status}"
    )
    ...
    combined_results = combine_and_score_v3(semantic_results, wordnet_results)
    ...
    # Update statistics
    if combined_results:
        avg_combined = sum(r["combined_score"] for r in combined_results) / len(
            combined_results
        )
        semantic_engine.stats["avg_combined_score"] = avg_combined

        # Count WordNet expansions
        wordnet_expansions = sum(
            len(r.get("matched_terms", [])) for r in combined_results
        )
    ...
    final_results = apply_cxd_filter(
        combined_results, function_filter, semantic_engine.cxd_classifier
    )
    ...
    logger.debug(
        f"🧠 MEMMIMIC HYBRID SEARCH v3.0 COMPLETE: {len(final_results)} results"
    )
...
# =============================================================================
# BEAUTIFUL OUTPUT FORMATTING v3.0
# =============================================================================


def format_memory_with_wordnet_info(
    memory_result: Dict[str, Any], index: int
) -> List[str]:
    """Format a memory result with beautiful WordNet hybrid search annotations for MemMimic."""
    ...
    # Enhanced symbols and indicators for MemMimic
    cxd_labels = {
        "CONTROL": "🎛️[CONTROL]",
        "CONTEXT": "🔗[CONTEXT]",
        "DATA": "📊[DATA]",
    ...
    combined_bar = "█" * int(combined_score * 10) + "░" * (
        10 - int(combined_score * 10)
    )
    semantic_bar = "█" * int(semantic_similarity * 10) + "░" * (
        10 - int(semantic_similarity * 10)
    )
    ...
    lines.append(
        f"   Memory: [{confidence_bar}] {confidence:.2f} | Combined: [{combined_bar}] {combined_score:.3f}"
    )
    lines.append(
        f"   Semantic: [{semantic_bar}] {semantic_similarity:.3f} | WordNet: [{lexical_bar}] {lexical_score:.3f}"
    )
    ...
    if bonus_info:
        lines.append(
            f"   🎯 BONUSES: {' | '.join(bonus_info)} | Method: {search_method}"
        )
    else:
    ...
    if original_matches > 0 or synonym_matches > 0:
        lines.append(
            f"   📚 WordNet: {original_matches} original + {synonym_matches} synonyms"
        )
...
def main():
    """🧠 MemMimic True Hybrid WordNet Memory Search - Command Line Interface"""
    import argparse

    parser = argparse.ArgumentParser(
        description="🧠 MemMimic True Hybrid WordNet Memory Search v3.0",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
EXAMPLES:
  memmimic_recall_cxd.py "machine learning" CONTROL 5
  memmimic_recall_cxd.py "analyze data" DATA 3 --db memmimic
  memmimic_recall_cxd.py "project context" ALL 10 --rebuild-index
...
MEMMIMIC HYBRID FEATURES v3.0:
  ...
  💎 Golden memories auto-briefing for self-discovery
        """,
    )
    ...
    parser.add_argument(
        "limit", nargs="?", type=int, default=5, help="Maximum results to return"
    )
    parser.add_argument(
        "--db",
        "--database",
        help="Database to search (memmimic, enhanced, legacy, backup, or filename)",
    )
    parser.add_argument(
        "--rebuild-index", action="store_true", help="Force rebuild of semantic index"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable verbose output"
    )
    parser.add_argument(
        "--stats", action="store_true", help="Show detailed search statistics"
    )
    ...
    try:
        ...
        function_filter = function_map.get(
            args.function_filter.upper(), args.function_filter.upper()
        )
        ...
        if args.rebuild_index:
            ...
            if memory_store:
                engine = MemMimicSemanticSearchEngine()
        ...
        # Perform TRUE hybrid search v3.0
        print(f"🧠 MemMimic True Hybrid WordNet Search v3.0", file=sys.stderr)
        print(
            f"🎯 Query: '{args.query}' | Filter: {function_filter} | Limit: {args.limit}",
            file=sys.stderr,
        )
        print(
            f"📚 WordNet: {'✅ Available' if wordnet_status else '❌ Unavailable (basic keywords only)'}",
            file=sys.stderr,
        )
        ...
        semantic_count = sum(
            1 for r in results if "semantic" in r.get("search_method", "")
        )
        wordnet_count = sum(
            1 for r in results if "wordnet" in r.get("search_method", "")
        )
        convergence_count = sum(
            1 for r in results if "convergence" in r.get("search_method", "")
        )
        original_term_count = sum(
            1 for r in results if r.get("original_matches", 0) > 0
        )

        if function_filter == "ALL":
            response_parts.append(
                f"🧠 MEMMIMIC TRUE HYBRID WORDNET SEARCH v3.0 ({len(results)} results){db_indicator}"
            )
        else:
            response_parts.append(
                f"🧠 MEMMIMIC {function_filter} HYBRID WORDNET SEARCH v3.0 ({len(results)} results){db_indicator}"
            )

        response_parts.append(
            f"🎯 Methods: {semantic_count} semantic + {wordnet_count} WordNet"
        )
        if convergence_count > 0:
            response_parts.append(
                f"🎯 CONVERGENCE: {convergence_count} results found by BOTH methods!"
            )
        if original_term_count > 0:
            response_parts.append(
                f"⭐ ORIGINAL TERMS: {original_term_count} results with exact query matches"
            )
        response_parts.append("")
        ...
        if args.stats and results:
            ...
            avg_combined = sum(r.get("combined_score", 0) for r in results) / len(
                results
            )
            avg_semantic = sum(r.get("semantic_similarity", 0) for r in results) / len(
                results
            )
            avg_lexical = sum(r.get("lexical_score", 0) for r in results) / len(results)
            total_convergence_bonus = sum(
                r.get("convergence_bonus", 0) for r in results
            )
            ...
            response_parts.append(
                f"   Average WordNet lexical score: {avg_lexical:.3f}"
            )
            response_parts.append(
                f"   Total convergence bonus: {total_convergence_bonus:.3f}"
            )
            response_parts.append(
                f"   Total original term bonus: {total_original_bonus:.3f}"
            )
            response_parts.append(f"   WordNet terms matched: {total_wordnet_terms}")
            response_parts.append(
                f"   Method distribution: {semantic_count} semantic, {wordnet_count} WordNet, {convergence_count} convergence"
            )
            response_parts.append("")
        ...
        # Footer
        wordnet_indicator = (
            " + NLTK WordNet" if wordnet_status else " (WordNet unavailable)"
        )
        response_parts.append(
            f"🧠 Powered by MemMimic v3.0 | True Hybrid Search = Semantic + Lexical{wordnet_indicator}"
        )
        ...
        print(
            f"💥 CRITICAL ERROR in MemMimic WordNet search: {str(e)}", file=sys.stderr
        )
        if args.verbose:
            import traceback
