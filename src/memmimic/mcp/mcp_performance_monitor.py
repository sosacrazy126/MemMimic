    ...
    def update(self, metrics: PerformanceMetrics):
        ...
        if metrics.duration_ms:
            ...
            if self.recent_durations:
                ...
                self.p95_duration_ms = statistics.quantiles(sorted_durations, n=20)[18] if len(sorted_durations) >= 20 else sorted_durations[-1]
                self.p99_duration_ms = statistics.quantiles(sorted_durations, n=100)[98] if len(sorted_durations) >= 100 else sorted_durations[-1]
        ...
        if metrics.memory_usage_mb:
            self.avg_memory_mb = ((self.avg_memory_mb * (self.total_calls - 1)) + metrics.memory_usage_mb) / self.total_calls
        
        if metrics.cpu_usage_percent:
            self.avg_cpu_percent = ((self.avg_cpu_percent * (self.total_calls - 1)) + metrics.cpu_usage_percent) / self.total_calls
...
class MCPPerformanceMonitor:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        if cache_dir:
            ...
            self.cache_dir = Path(__file__).parent.parent / "memmimic_cache" / "mcp_metrics"
        ...
        with self._lock:
            if operation_name:
                if operation_name in self.operation_stats:
                    return self._format_operation_stats(self.operation_stats[operation_name])
                else:
                    return {}
            else:
                return {
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_response_time = sum(stats.avg_duration_ms for stats in self.operation_stats.values()) / len(self.operation_stats)
            ...
            recommendations = []
            if error_rate > self.performance_thresholds['error_rate_threshold']:
                recommendations.append("High error rate detected - investigate failed operations")
            if avg_response_time > self.performance_thresholds['warning_duration_ms']:
                recommendations.append("Response times elevated - consider optimization")
    ...
    def _check_performance_alerts(self, metrics: PerformanceMetrics):
        ...
        if not metrics.success:
            self.logger.warning(f"Operation {metrics.operation_name} failed: {metrics.error_message}")
        
        if metrics.duration_ms:
            if metrics.duration_ms > self.performance_thresholds['critical_duration_ms']:
                self.logger.error(f"CRITICAL: {metrics.operation_name} took {metrics.duration_ms:.1f}ms")
            elif metrics.duration_ms > self.performance_thresholds['warning_duration_ms']:
                self.logger.warning(f"SLOW: {metrics.operation_name} took {metrics.duration_ms:.1f}ms")
        
        if metrics.memory_usage_mb:
            if metrics.memory_usage_mb > self.performance_thresholds['memory_critical_mb']:
                self.logger.error(f"CRITICAL: {metrics.operation_name} used {metrics.memory_usage_mb:.1f}MB memory")
            elif metrics.memory_usage_mb > self.performance_thresholds['memory_warning_mb']:
                self.logger.warning(f"HIGH MEMORY: {metrics.operation_name} used {metrics.memory_usage_mb:.1f}MB")
    ...
    def _load_metrics(self):
        ...
        try:
            ...
            for name, stats_data in data.get('operation_stats', {}).items():
                ...
                stats.avg_user_satisfaction = stats_data.get('avg_user_satisfaction', 0.0)
                
                self.operation_stats[name] = stats
            
            self.logger.info(f"Loaded metrics for {len(self.operation_stats)} operations")
    ...
    def _export_prometheus_metrics(self) -> str:
        ...
        lines.append("# HELP mcp_operation_duration_seconds Time spent on MCP operations")
        ...
        lines.append("# HELP mcp_operation_errors_total Total number of MCP operation errors")
        lines.append("# TYPE mcp_operation_errors_total counter")
        
        # Export metrics for each operation
        for name, stats in self.operation_stats.items():
            # Operation totals
            lines.append(f'mcp_operation_total{{operation="{name}"}} {stats.total_calls}')
            lines.append(f'mcp_operation_errors_total{{operation="{name}"}} {stats.failed_calls}')
            
            # Duration metrics
            lines.append(f'mcp_operation_duration_seconds{{operation="{name}",quantile="0.5"}} {stats.median_duration_ms/1000}')
            lines.append(f'mcp_operation_duration_seconds{{operation="{name}",quantile="0.95"}} {stats.p95_duration_ms/1000}')
            lines.append(f'mcp_operation_duration_seconds{{operation="{name}",quantile="0.99"}} {stats.p99_duration_ms/1000}')
            ...
            try:
                ...
                if isinstance(result, dict):
                    if 'memories_processed' in result:
                        extra_metrics['memories_processed'] = result['memories_processed']
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        monitor.finish_operation(op_id, success=True, memories_processed=10, results_returned=5)
