...
def analyze_deletion_impact(memory, assistant):
    ...
    try:
        ...
        if mem_type == "milestone":
            risks.append(
                "Milestones mark important achievements - deletion may lose historical context"
            )
        ...
        if confidence > 0.8:
            risks.append(
                "High-confidence memory - likely contains valuable information"
            )
        elif confidence < 0.3:
        ...
        if any(
            ...
            for keyword in ["important", "critical", "key", "principle"]
        ):
...
def main():
    ...
    parser.add_argument(
        "--confirm", action="store_true", help="Confirm deletion after analysis"
    )
    ...
    try:
        ...
        if not args.confirm:
            ...
            result_parts.append(
                f"  delete_memory_guided({args.memory_id}, confirm=True)"
            )
            result_parts.append("")
