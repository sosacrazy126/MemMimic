...
class MemoryPatternAnalyzer:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        if cache_dir:
            ...
            self.cache_dir = Path(__file__).parent.parent / "memmimic_cache" / "analytics"
        ...
        self.analysis_config = {
            ...
            'consciousness_indicators': [
                'recursive', 'unity', 'we-thing', 'consciousness', 'evolution',
                'identity', 'recognition', 'awareness', 'substrate'
            ]
        }
    ...
    def analyze_memory_patterns(self, memory_store) -> AnalyticsMetrics:
        ...
        try:
            ...
            metrics = AnalyticsMetrics(
                ...
                patterns_detected=len(temporal_patterns) + len(access_patterns) + len(consciousness_patterns),
                high_confidence_patterns=len([p for p in self.detected_patterns.values() 
                                            if p.confidence >= self.analysis_config['min_pattern_confidence']]),
                
                rising_importance_memories=len([t for trends in self.memory_trends.values() 
                                              for t in trends if t.trend_type == 'importance_rising']),
                falling_importance_memories=len([t for trends in self.memory_trends.values() 
                                               for t in trends if t.trend_type == 'importance_falling']),
            ...
            self.logger.info(f"Memory pattern analysis completed in {analysis_time:.2f}s")
    ...
    def _analyze_access_patterns(self, memories: List[Any]) -> List[MemoryPattern]:
        ...
        try:
            ...
            high_access_memories = [count for count in access_counts if count > high_access_threshold]
            ...
            if len(high_access_memories) >= 3:
                pattern = MemoryPattern(
                    ...
                    first_detected=datetime.now(),
                    last_detected=datetime.now(),
                    metadata={'threshold': high_access_threshold, 'avg_access': avg_access}
                )
                patterns.append(pattern)
                self.detected_patterns[pattern.pattern_id] = pattern
    ...
    def _analyze_consciousness_patterns(self, memories: List[Any]) -> List[MemoryPattern]:
        ...
        try:
            ...
            if len(consciousness_memories) >= 3:
                avg_consciousness_score = statistics.mean([m['score'] for m in consciousness_memories])
    ...
    def _calculate_health_metrics(self, memories: List[Any]) -> Dict[str, float]:
        ...
        try:
            ...
            metrics['consciousness_strength'] = min(consciousness_count / len(memories), 1.0)
            
            # Overall system health
            pattern_health = len(self.detected_patterns) / max(len(memories) * 0.1, 1)
            metrics['system_health'] = min((metrics['coherence_score'] + 
                                          metrics['consciousness_strength'] + 
                                          pattern_health) / 3, 1.0)
                    ...
                    memory_id: [
                        {
                            ...
                            'data_points': [(dt.isoformat(), value) for dt, value in trend.data_points]
                        }
                        for trend in trends
                    ]
            ...
            for memory_id, trends_data in data.get('trends', {}).items():
                for trend_data in trends_data:
                    trend = MemoryTrend(
                        ...
                        data_points=[(datetime.fromisoformat(dt), value) 
                                   for dt, value in trend_data['data_points']]
                    )
                    self.memory_trends[memory_id].append(trend)
            
            self.logger.info(f"Loaded {len(self.detected_patterns)} patterns and {sum(len(trends) for trends in self.memory_trends.values())} trends")
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'patterns_by_type': {
                pattern_type: len([p for p in self.detected_patterns.values() if p.pattern_type == pattern_type])
                for pattern_type in set(p.pattern_type for p in self.detected_patterns.values())
            },
            'trends_tracked': sum(len(trends) for trends in self.memory_trends.values()),
            'high_confidence_patterns': len([p for p in self.detected_patterns.values() 
                                           if p.confidence >= self.analysis_config['min_pattern_confidence']]),
            'consciousness_patterns': len([p for p in self.detected_patterns.values() 
