...
class ScoringWeights:
    ...
    def validate(self) -> bool:
        ...
        total = (self.cxd_classification + self.access_frequency + 
                self.recency_temporal + self.confidence_quality + self.memory_type)
...
class ImportanceScorer:
    ...
    def calculate_importance(self, metrics: MemoryMetrics) -> Tuple[float, Dict[str, float]]:
        """
        ...
        components['weighted_components'] = {
            ...
            'access_weighted': components['access_score'] * self.weights.access_frequency,
            'recency_weighted': components['recency_score'] * self.weights.recency_temporal,
            'confidence_weighted': components['confidence_score'] * self.weights.confidence_quality,
    ...
    def _calculate_recency_score(self, metrics: MemoryMetrics) -> float:
        ...
        if metrics.last_access_time:
            ...
            recency_score = math.exp(-hours_since_access / (self.recency_half_life_days * 24))
        else:
            recency_score = 0.0
        
        # Calculate temporal decay based on creation time
        hours_since_creation = (now - metrics.created_at).total_seconds() / 3600
        temporal_decay = math.exp(-hours_since_creation * self.temporal_decay_rate / (24 * 365))
        
        # Combine recency and temporal factors
        combined_score = (recency_score * 0.7) + (temporal_decay * 0.3)
        
        return max(combined_score, 0.01)  # Minimum score to keep very old memories accessible
    ...
    def _apply_contextual_modifiers(self, base_score: float, metrics: MemoryMetrics) -> float:
    ...
    def _estimate_cxd_importance_from_content(self, content: str) -> float:
        ...
        control_keywords = ['search', 'find', 'filter', 'decide', 'choose', 'select', 'control']
        context_keywords = ['relate', 'connect', 'context', 'because', 'since', 'therefore', 'relationship']
        data_keywords = ['store', 'save', 'data', 'information', 'process', 'calculate', 'compute']
        ...
        if context_score >= control_score and context_score >= data_score:
            return self.cxd_importance_map[CXDFunction.CONTEXT] * 0.7  # Reduced confidence
    ...
    def _normalize_weights(self):
        ...
        total = (self.weights.cxd_classification + self.weights.access_frequency + 
                self.weights.recency_temporal + self.weights.confidence_quality + 
...
def create_metrics_from_memory_data(memory_data: Dict[str, Any]) -> MemoryMetrics:
    ...
    return MemoryMetrics(
        ...
        last_access_time=datetime.fromisoformat(memory_data['last_access_time']) if memory_data.get('last_access_time') else None,
        access_frequency=memory_data.get('access_frequency', 0.0),
        cxd_function=CXDFunction(memory_data['cxd_function']) if memory_data.get('cxd_function') else None,
...
def calculate_importance_for_memory(memory_data: Dict[str, Any], 
                                   scorer: Optional[ImportanceScorer] = None) -> Tuple[float, Dict[str, float]]:
