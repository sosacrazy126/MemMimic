...
class MemoryConsolidator:
    ...
    def _create_consolidation_schema(self):
        ...
        with self._get_connection() as conn:
            ...
            for col_name, col_def in missing_columns:
                try:
                    conn.execute(
                        f"ALTER TABLE consolidation_groups ADD COLUMN {col_name} {col_def}"
                    )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_memory_group_membership_memory ON memory_group_membership(memory_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_memory_group_membership_group ON memory_group_membership(group_id)"
            )
            try:
                conn.execute(
    ...
    def _get_eligible_memories(self, conn) -> List[Dict[str, Any]]:
        ...
        min_age = (
            datetime.now() - timedelta(hours=self.config.min_memory_age_hours)
        ).isoformat()
        ...
        """,
            (
                ...
                (
                    datetime.now()
                    - timedelta(hours=self.config.consolidation_cooldown_hours)
                ).isoformat(),
            ),
        )
    ...
    def _calculate_semantic_similarity(
        ...
        if (
            memory1.get("cxd_function")
            and memory2.get("cxd_function")
            and memory1["cxd_function"] == memory2["cxd_function"]
        ):
            score += 0.3
    ...
    def _consolidate_duplicate_group(
        self, conn, duplicate_group: List[Dict[str, Any]], stats: Dict[str, int]
    ):
        ...
        primary_memory = max(
            duplicate_group, key=lambda m: (m["importance_score"], m["created_at"])
        )
        ...
        for memory in duplicate_group:
            self._add_memory_to_group(
                conn, memory["id"], group_id, memory["id"] == primary_memory["id"]
            )

        # Mark non-primary memories as consolidated
        non_primary_ids = [
            m["id"] for m in duplicate_group if m["id"] != primary_memory["id"]
        ]
        if non_primary_ids:
        ...
        self._record_consolidation_audit(
            ...
            group_id,
            [m["id"] for m in duplicate_group],
            [1.0] * len(duplicate_group),
        )

        stats["groups_created"] += 1
        stats["memories_consolidated"] += len(duplicate_group) - 1

    def _consolidate_similarity_group(
        self, conn, similarity_group: List[Dict[str, Any]], stats: Dict[str, int]
    ):
        ...
        group_id = self._create_consolidation_group(
            ...
            group_hash,
            primary_memory["id"],
            len(similarity_group),
            "similarity_consolidation",
            avg_similarity,
        )
        ...
        self._record_consolidation_audit(
            ...
            "similarity_consolidation",
            group_id,
            [m["id"] for m in similarity_group],
            similarities,
        )
    ...
    def _create_consolidation_group(
        self,
        conn,
        group_hash: str,
    ...
    def _calculate_contribution_score(
        self, memory: Dict[str, Any], group: List[Dict[str, Any]]
    ) -> float:
    ...
    def _update_last_consolidation_time(self, conn):
        ...
        self._record_consolidation_audit(conn, "full_consolidation", None, [], [])
...
def create_memory_consolidator(
    db_path: str, config: Optional[ConsolidationConfig] = None
) -> MemoryConsolidator:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(
            f"  Average group size: {status['group_statistics']['avg_group_size']:.1f}"
        )
        print(
            f"  Average similarity: {status['group_statistics']['avg_similarity']:.3f}"
        )
