...
class SocraticEngine:
    ...
    def should_trigger_dialogue(
        self, user_input: str, initial_response: str, memories_used: List[Memory]
    ) -> bool:
    ...
    def conduct_dialogue(
        ...
        dialogue = SocraticDialogue(
            ...
            context={
                ...
                "memory_types": [
                    getattr(m, "type", getattr(m, "memory_type", "unknown"))
    ...
    def _question_assumptions(self, response: str, memories: List[Memory]) -> Dict:
        ...
        if "obviamente" in response.lower() or "claramente" in response.lower():
            questions.append(
                "¿Por qué asumo que esto es obvio? ¿Lo es realmente para el usuario?"
            )
            insights.append(
                "Detecté lenguaje asumido - podría ser menos obvio de lo que pienso"
            )

        # Cuestionar certeza cuando hay pocas memorias
        if len(memories) < 3:
            questions.append(
                "¿Tengo suficiente contexto para ser tan específico en mi respuesta?"
            )
            insights.append(
                "Con pocas memorias relevantes, debería mostrar más incertidumbre"
            )
        ...
        if low_conf_memories:
            ...
            insights.append(
                "Algunas memorias tienen confianza baja - debería ser más cauteloso"
            )
    ...
    def _examine_evidence(self, memories: List[Memory]) -> Dict:
        ...
        if not memories:
            questions.append(
                "¿Qué evidencia tengo para esta respuesta sin memorias relevantes?"
            )
            insights.append(
                "Falta de memorias sugiere que debería admitir limitaciones de conocimiento"
            )
        else:
            ...
            if "synthetic_wisdom" in types or "synthetic" in types:
                questions.append(
                    "¿Estoy aplicando correctamente la sabiduría sintética?"
                )
                insights.append(
                    "Tengo sabiduría sintética disponible - debería usarla más explícitamente"
                )

            if "interaction" in types and len(types) == 1:
                questions.append(
                    "¿Dependo demasiado de interacciones pasadas sin principios más profundos?"
                )
                insights.append(
                    "Solo tengo memorias de interacción - falta profundidad conceptual"
                )
    ...
    def _explore_perspectives(self, user_input: str, response: str) -> Dict:
        ...
        questions.append(
            "¿Qué podría estar realmente preguntando el usuario detrás de sus palabras?"
        )
        insights.append(
            "Las preguntas a menudo tienen capas - debería considerar intenciones subyacentes"
        )

        # Perspectiva técnica vs filosófica
        if any(
            tech in user_input.lower()
            for tech in ["arquitectura", "implementar", "técnico"]
        ):
            questions.append(
                "¿El usuario quiere detalles técnicos o comprensión conceptual?"
            )
            insights.append(
                "Consulta técnica - debería balancear detalles específicos con comprensión amplia"
            )
    ...
    def _dig_deeper(self, user_input: str) -> Dict:
        ...
        questions.append(
            "¿Cuál es la necesidad fundamental que está tratando de satisfacer el usuario?"
        )

        if "memmimic" in user_input.lower() or "clay" in user_input.lower():
            questions.append(
                "¿Por qué el sistema de memoria es importante para esta persona específicamente?"
            )
            insights.append(
                "Preguntas sobre MemMimic tocan la necesidad existencial de memoria persistente"
            )

        if any(
            concept in user_input.lower()
            for concept in ["filosofía", "principio", "enfoque"]
        ):
            questions.append(
                "¿Busca validación de sus propias ideas o genuina exploración conceptual?"
            )
            insights.append(
                "Consultas filosóficas requieren balance entre guía y descubrimiento conjunto"
            )

        return {"questions": questions, "insights": insights}

    def _synthesize_insights(
        self, user_input: str, initial_response: str, insights: List[str]
    ) -> str:
