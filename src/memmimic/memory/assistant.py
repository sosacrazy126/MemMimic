...
class ContextualAssistant:
    ...
    def _refine_response(
        self, initial_response: str, socratic_result: Optional[Dict]
    ) -> str:
    ...
    def _save_learning(
        self, user_input: str, final_response: str, socratic_result: Optional[Dict]
    ):
        ...
        interaction_memory = Memory(
            ...
            ),  # Mayor confianza si hubo auto-cuestionamiento
        )
        ...
        for memory in memories:
            ...
            if any(
                ...
                process["connections_made"].append(
                    f"Continuidad detectada con memoria previa: {memory.type}"
                )

            # Detectar preguntas de profundización
            if any(
                trigger in input_lower
                for trigger in ["más", "detalles", "explica", "cómo", "por qué"]
            ):
    ...
    def _generate_response(
        self, user_input: str, thought_process: Dict, memories: List[Memory]
    ) -> str:
        ...
        if synthetic_memories:
            ...
            if any(
                ...
                for keyword in ["filosofía", "principio", "arquitectura", "origen"]
            ):
        ...
        if any(
            term in input_lower
            for term in ["clay", "proyecto", "estado", "memoria", "asistente"]
        ):
        ...
        if any(
            term in input_lower
            for term in ["más", "detalles", "continúa", "sigue", "explica mejor"]
        ):
    ...
    def _respond_with_context(self, user_input: str, memories: List[Memory]) -> str:
        ...
        )  # Top 3 memorias más relevantes

        # Construir respuesta contextual
        context_summary = self._extract_key_context(memory_content)

        response = (
            f"Considerando nuestras conversaciones anteriores sobre {context_summary}, "
        )
    ...
    def _calculate_confidence(
        self, memories: List[Memory], socratic_result: Optional[Dict]
    ) -> float:
