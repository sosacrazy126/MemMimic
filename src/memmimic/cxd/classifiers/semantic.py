...
class SemanticCXDClassifier(CXDClassifier):
    ...
    def __init__(
        ...
        self.example_provider = (
            example_provider or self._create_default_example_provider()
        )
        ...
        self._examples_by_function: Optional[
        ...
        logger.info(
            f"Initialized SemanticCXDClassifier with {self.embedding_model.model_name}"
        )
    ...
    def _build_index(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Semantic index built in {build_time:.2f}s with {len(all_texts)} examples"
            )
    ...
    def _aggregate_function_scores(
        self, similarities: np.ndarray, indices: np.ndarray
    ) -> Dict[CXDFunction, float]:
        """
        ...
        for function, sims in function_similarities.items():
            if sims:
                ...
                weighted_avg = np.average(
                    sims_array, weights=weights[: len(sims_array)]
                )
    ...
    def _create_cxd_tags(
        self, function_scores: Dict[CXDFunction, float], original_text: str
    ) -> List[CXDTag]:
        """
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if total_classifications > 0:
            for function, count in self.stats["function_detections"].items():
                stats[f"{function.value}_detection_rate"] = (
        ...
        for tag in result.tags:
            explanation["function_analysis"][tag.function.value] = {
                ...
                "semantic_confidence": getattr(
                    tag, "semantic_confidence", tag.confidence
                ),
