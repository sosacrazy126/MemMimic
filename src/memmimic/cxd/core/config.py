...
class PathsConfig(BaseSettings):
    ...
    canonical_examples: Path = Field(
        default=Path("./config/canonical_examples.yaml"),
        description="Canonical examples file",
    )
    ...
    @field_validator(
        "cache_dir", "log_dir", "models_dir", mode="before"
    )  # mode='before' para replicar comportamiento de validator pre-v2
    @classmethod
...
class EmbeddingConfig(BaseSettings):
    ...
    fallback_dimension: int = Field(
        default=384, ge=50, le=4096, description="Fallback dimension for mock models"
    )
...
class MockModelConfig(BaseSettings):
    ...
    dimension: int = Field(
        default=384, ge=50, le=4096, description="Mock model dimension"
    )
...
class ThresholdsConfig(BaseSettings):
    ...
    concordance: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Meta-classification concordance threshold",
    )
    semantic: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Semantic similarity minimum threshold"
    )
    confidence_min: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Minimum confidence for classification"
    )
...
class SearchConfig(BaseSettings):
    ...
    timeout_ms: int = Field(
        default=1000, ge=100, description="Search timeout in milliseconds"
    )
...
class FusionConfig(BaseSettings):
    ...
    lexical_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Weight for lexical classifier"
    )
    semantic_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Weight for semantic classifier"
    )
    semantic_override: bool = Field(
        default=True, description="Enable semantic override in low concordance"
    )
    ...
    def validate_weights_sum(self) -> "FusionConfig":
        ...
        if abs(total - 1.0) > 0.01:  # Allow small floating point errors
            raise ValueError(
                f"Lexical and semantic weights must sum to 1.0 (got {total})"
            )
...
class FeaturesConfig(BaseSettings):
    ...
    cache_persistence: bool = Field(
        default=True, description="Enable cache persistence"
    )
    ...
    auto_optimization: bool = Field(
        default=False, description="Enable automatic optimization"
    )
    fine_tuning: bool = Field(
        default=False, description="Enable fine-tuning capabilities"
    )
...
class PerformanceConfig(BaseSettings):
    ...
    batch_size: int = Field(
        default=32, ge=1, le=1000, description="Batch processing size"
    )
    max_concurrent_requests: int = Field(
        default=100, ge=1, description="Max concurrent requests"
    )
    max_cache_size_mb: int = Field(
        default=1024, ge=100, description="Max cache size in MB"
    )
    cache_cleanup_interval: int = Field(
        default=3600, ge=300, description="Cache cleanup interval in seconds"
    )
    classification_timeout: int = Field(
        default=30, ge=1, description="Classification timeout in seconds"
    )
    index_build_timeout: int = Field(
        default=300, ge=10, description="Index build timeout in seconds"
    )
    model_load_timeout: int = Field(
        default=120, ge=10, description="Model load timeout in seconds"
    )
    num_threads: int = Field(
        default=1, ge=0, le=64, description="Number of threads (0=auto-detect)"
    )
    thread_pool_size: int = Field(
        default=4, ge=1, le=32, description="Thread pool size"
    )
...
class LoggingConfig(BaseSettings):
    ...
    log_classifications: bool = Field(
        default=True, description="Log classification events"
    )
    ...
    max_input_length_logged: int = Field(
        default=200, ge=50, description="Max input length to log"
    )
...
class ValidationConfig(BaseSettings):
    ...
    max_text_length: int = Field(
        default=10000, ge=100, description="Maximum text length"
    )
    min_text_length: int = Field(default=1, ge=1, description="Minimum text length")
    allowed_languages: List[str] = Field(
        default_factory=list, description="Allowed languages (empty=all)"
    )
    enable_regression_testing: bool = Field(
        default=True, description="Enable regression testing"
    )
    golden_dataset_path: Path = Field(
        default=Path("./tests/data/golden_dataset.yaml"),
        description="Golden dataset path",
    )
...
class CLIConfig(BaseSettings):
    ...
    output_format: OutputFormat = Field(
        default=OutputFormat.TABLE, description="Default output format"
    )
    ...
    default_verbosity: int = Field(
        default=1, ge=0, le=3, description="Default verbosity level"
    )
...
class APIConfig(BaseSettings):
    ...
    rate_limit_rpm: int = Field(
        default=1000, ge=1, description="Rate limit (requests per minute)"
    )
...
class ExperimentalConfig(BaseSettings):
    ...
    neural_classifier: bool = Field(
        default=False, description="Enable neural classifier"
    )
    transformer_fine_tuning: bool = Field(
        default=False, description="Enable transformer fine-tuning"
    )
    active_learning: bool = Field(default=False, description="Enable active learning")
    uncertainty_quantification: bool = Field(
        default=False, description="Enable uncertainty quantification"
    )
    image_text_fusion: bool = Field(
        default=False, description="Enable image-text fusion"
    )
    audio_text_fusion: bool = Field(
        default=False, description="Enable audio-text fusion"
    )
    cluster_processing: bool = Field(
        default=False, description="Enable cluster processing"
    )
...
class CXDConfig(BaseSettings):
    ...
    description: str = Field(
        default="CXD Classifier Configuration", description="Configuration description"
    )
    ...
    def save_to_yaml(self, config_path: Union[str, Path]) -> None:
        ...
        config_dict = self.model_dump(
            mode="python"
        )  # mode='python' para obtener tipos nativos de Python como Path
    ...
    def load_with_overrides(
        ...
        **kwargs,
        ...
        final_config_data = {
            **yaml_values,
            **kwargs,
        }  # kwargs sobreescriben yaml_values
        return cls(
            **final_config_data
        )  # Pydantic-Settings cargará ENV y .env sobre esto
        ...
        if cache_dir.exists():
            try:
                ...
                if free_space_mb < self.performance.max_cache_size_mb * 2:
                    warnings.append(
                        f"Low disk space for cache: {free_space_mb}MB available"
                    )
            except Exception:
                pass
        return warnings
...
def load_config_from_file(
    ...
    if not env_override:
        ...
        temp_model_config = SettingsConfigDict(
            env_file=None, extra="ignore"
        )  # Intenta deshabilitar carga de ENV
    ...
    return CXDConfig(
        **yaml_values
    )  # Pydantic-Settings cargará ENV y .env sobre los valores de yaml_values
