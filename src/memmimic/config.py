...
class ConfigLoader:
    ...
    def load_config(self) -> MemMimicConfig:
        ...
        if not self._validate_config(self._config):
            self.logger.warning("Configuration validation failed, using validated defaults")
        ...
        for path in config_paths:
            ...
            if expanded_path.exists():
                try:
                    ...
                    self.logger.error(f"Failed to load config from {expanded_path}: {e}")
                    continue
    ...
    def _parse_config(self, config_data: Dict[str, Any]) -> MemMimicConfig:
        ...
        for policy_name, policy_data in {**default_policies, **retention_config}.items():
    ...
    def _validate_config(self, config: MemMimicConfig) -> bool:
        ...
        try:
            ...
            # Validate pool sizes
            if config.active_memory_pool.target_size >= config.active_memory_pool.max_size:
    ...
    def _get_default_config(self) -> MemMimicConfig:
        ...
        return MemMimicConfig(
            ...
            scoring_weights=ScoringWeightsConfig(),
            retention_policies={
                'synthetic_wisdom': RetentionPolicyConfig('permanent', 0.2, 'permanent'),
